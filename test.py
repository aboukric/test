...
def process_data(data,cntry='Alberta',window=3):
    conf_ts = data
    conf_ts_cntry = conf_ts[conf_ts['Province/State']==cntry]
...
def get_cntry_total(df,cntry='Alberta'):
    return df[df['Province/State']==cntry].iloc[:,-1].sum()
...
def fig_world_trend(cntry='Alberta',window=3):
...
def get_country_list():
    return covid_conf_ts['Province/State'].astype(str)
...
def graph1():
    return dcc.Graph(id='graph1',figure=fig_world_trend('Alberta'))
...
def generate_cards(cntry='Alberta'):
    conf_cntry_total = get_cntry_total(covid_conf_ts,cntry)
    dead_cntry_total = get_cntry_total(covid_dead_ts,cntry)
    recv_cntry_total = get_cntry_total(covid_recv_ts,cntry)
    cards = html.Div(
